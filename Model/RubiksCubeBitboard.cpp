...
private:
    ...
    void rotateFace(int ind) {
        ...
        side =(side >> (8 * 6));  //storing the last 16 bits
        bitboard[ind] = (bitboard[ind] << 16) | (side); //setting the last 16 bits in
        //the rotated configuration
    }
    //what is this
    void rotateSide(const int s1, const int  s1_1, const int  s1_2, const int  s1_3, const int  s2, const int  s2_1, const int  s2_2, const int  s2_3) {

        const uint64_t clr1 = (bitboard[s2] & (one_8 << (8 * s2_1))) >> (8 * s2_1);
        const uint64_t clr2 = (bitboard[s2] & (one_8 << (8 * s2_2))) >> (8 * s2_2);
        const uint64_t clr3 = (bitboard[s2] & (one_8 << (8 * s2_3))) >> (8 * s2_3);
