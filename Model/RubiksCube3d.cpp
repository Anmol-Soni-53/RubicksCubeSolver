//
// Created by hp on 14-06-2024.
//

#include "GenericRubiksCube.h"

class RubiksCube3d: public GenericRubiksCube {
private:
...
public:
    //initialise Cube as 3-D Array
    char cube[6][3][3]{};

    RubiksCube3d() { //constructer
        //generating faces of cube of all colors
        for(int i=0;i<6;i++) {
            for(int row=0;row<3;row++) {
                for(int col=0;col<3;col++) {
                    cube[i][row][col]=getColorLetter(COLOR(i));
                }
            }
        }
    }
    ...
    bool isSolved() const override {
        for (int i = 0; i < 6; i++) {
            for (int row = 0; row < 3; row++) {
                for (int col = 0; col < 3; col++) {
                    if (this->cube[i][row][col] == getColorLetter(COLOR(i))) continue;
                    return false;
                }
            }
        }
    ...
    GenericRubiksCube &uPrime() override {
        // performing u operation 3 time
        // result im u' operation
        this->u();
        this->u();
        this->u();
    ...
    //TASK 5 OPERATOR OVERLOADING AND CUSTOM HASH STRUCTURE.
...
struct Hash3d {
    ...
    size_t operator()(const RubiksCube3d &r1) const {
        ...
        //This is a standard library function object that generates a hash value for its argument.
